...
impl JwtVerifier {
    ...
    pub async fn verify<Claims: DeserializeOwned>(
        ...
        let jwks = fetch_jwt(&format!("{}/.well-known/jwks.json", self.domain)).await?;
        Ok(verify_jwt(jwt, &jwks, aud).await?)
    }
}
...
mod tests {
    ...
    async fn test_verify_jwt() {
        ...
        let jwks = fetch_jwt(&format!("{}/.well-known/jwks.json", "http://localhost:1234")).await.unwrap();
